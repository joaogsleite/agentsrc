...
async function payload(module: ResolvedModule): Promise<string[] | Error> {
  ...
  for (const file of result) if (!isSafeRelativePath(file) || file === ".agentsrc.json" || file.startsWith("config/") || file.startsWith("docs/") || file.startsWith("sessions/") || file.startsWith("state/")) return fail(`Module ${module.manifest.name} has reserved destination ${file}`)
  return result
}
